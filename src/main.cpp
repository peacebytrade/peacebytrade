...
// Copyright (c) 2014 The peacebytrade developers
...
const string strMessageMagic = "peacebytrade Signed Message:\n";
...
// 
//
// Insert Coin Rewards as per group discussion
//
//
//
// The Focus of this coin is to foster trade amongst people 
// as opposed to Mining and get rich fast speculation. 
// Mining though is important in the 2nd phase. 
// Short term speculators should be crowded out, at least initially.
// => Premine as a Monetary Management tool allocated for the purposes
//    => of fostering Trade, Charity and stability = cover cost 
//	for distribution setup and ongoing development
// Money supply could potentially be used to controll price 
// in order not to damage the coin value (just the option, should do the job)
// Release plan includes a 7 year scheduled release (needs to be made public) 

if (nHeight <= 165)
    {           // 165 mio. Coins premine in 1 mio chunks
           nSubsidy = 1000000;
           return nSubsidy * COIN;
    }
// There is a 3 months grace period with low mining rewards
// preventing whales coming in and destabilize pricing through pump and dump
// => Objective: get away from hording and reward network effect 
//
// Q1 Year 1 - after Release - most likely number of users is still small, so
// 
        if (nHeight >165 && nHeight <= 86400)
    {
        nSubsidy = 10;
        return nSubsidy * COIN;
    }
// Q2 Year 1 - reflect the higher potential, once userbase and
// awareness has been successfully created
    if (nHeight > 86400 && nHeight <= 175680)
    {
        nSubsidy = 20;
        return nSubsidy * COIN;
    }
// Q3 Year 1 - reflect single user dilution by larger miners and
//pools.
        // Idea is to inflate currency against existing coins at this point.
//Discourage hording, focus on trade.
        // Question: Is money supply balanced against real-life use in
//commercial trade activity. (The premine can only balance demand, not supply)
    if (nHeight > 175680 && nHeight <= 351360)
    {
        nSubsidy = 30;
        return nSubsidy * COIN;
    }
    // Year 1 - Existing experience with the coin and hopefully many
//using it in order to trade real goods and services
    // This is when the PB-"Trade" hype should start and demand for coins
//would hopefully increase -> increase money supply
        // also start to compensate for coins lost from the money base in
//crashed wallets, lost passwords, etc.
        if (nHeight > 351360 && nHeight <= 527040)
    {
        nSubsidy = 40;
        return nSubsidy * COIN;
    }
// Year 2 - Max reward - Attract more companies to accept the coin
// successfull trading activity will demand stability - Miners
//should now be attracted to create strong network of nodes
    if (nHeight > 527040 && nHeight <= 702720)
    {
        nSubsidy = 50;
        return nSubsidy * COIN;
    }
// Year 3 - start to shrink supply countering bubble effect
// hopefully still attractive
    if (nHeight > 702720 && nHeight <= 878400)
    {
        nSubsidy = 60;
        return nSubsidy * COIN;
    }
    // Year 4 - Lower supply - By year 4 money supply should be
//regulated by premine releases to the community
        // nodes and miners should now still be part of the game, but
//stability and reach should be established and community should refocus on
//trade
    if (nHeight > 878400 && nHeight <= 1054080)
    {
        nSubsidy = 80;
        return nSubsidy * COIN;
    }
        // Year 5 - global acceptance - new markets - reflect needs of
//emerging economies and Charities operating in these areas
    if (nHeight > 1054080 && nHeight <= 1229760)
    {
        nSubsidy = 100;
        return nSubsidy * COIN;
    }
    // Year 6 - Smooth out new currency generation
    if (nHeight > 1229760 && nHeight <= 1405440)
    {
        nSubsidy = 120;
        return nSubsidy * COIN;
    }
        // 7 year life cycle - last strong rewards for miners
    if (nHeight > 1405440 && nHeight <= 1581120)
    {
        nSubsidy = 100;
        return nSubsidy * COIN;
    }
        // Year 8 - money supply to start exceeding premined 50 percent
        // By this time premined should be already in the market
        if (nHeight > 1581120 && nHeight <= 1756800)
    {
        nSubsidy = 100;
        return nSubsidy * COIN;
    }
        // Year 9 - get last ones hooked up on trading as a tool for peace
        if (nHeight > 1756800 && nHeight <= 1932480)
    {
        nSubsidy = 80;
        return nSubsidy * COIN;
    }
        if (nHeight > 1932480 && nHeight <= 2108160)
    {
        nSubsidy = 80;
        return nSubsidy * COIN;
    }
        if (nHeight > 2108160 && nHeight <= 2283840)
    {
        nSubsidy = 60;
        return nSubsidy * COIN;
    }
        if (nHeight > 2283840 && nHeight <= 2459520)
    {
        nSubsidy = 30;
        return nSubsidy * COIN;
    }
        // from year 9 on only compensate for losses in money supply
//available amid technical issues, burnt coins, etc.
    if (nHeight > 2459520)
    {
        nSubsidy = 5;

        unsigned int nHeightTemp = nHeight - 70037;
        nSubsidy >>= (nHeightTemp / 131400);
        return nSubsidy * COIN;
    }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("peacebytradeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("peacebytradeMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("peacebytrade miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    if (pindexBest->nHeight < DIFF_SWITCH_BLOCK) 
        {
            ...
            {
                printf("peacebytrade miner terminated\n");
                throw;
            }
