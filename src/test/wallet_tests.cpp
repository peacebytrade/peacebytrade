...
static void add_coin(int64 nValue, int nAge = 6*24, bool fIPBTomMe = false, int nInput=0)
{
    ...
    if (fIPBTomMe)
    {
        // IPBTomMe() returns (GetDebit() > 0), and GetDebit() is 0 if vin.empty(),
        // so stop vin being empty, and cache a non-zero Debit to fake out IPBTomMe()
        wtx->vin.resize(1);
